...
namespace Server
{
    ...
    class Server
    {
        ...
        public static void sendEvents(String user, Socket sock)
        {
            //get friends
            String events = db.getEvents(user);
            byte[] data = Encoding.ASCII.GetBytes(events);
            byte[] len = new byte[2];
            len[0] = (byte)(data.Length >> 8);
            len[1] = (byte)(data.Length & 0xFF);
        ...
        public static void sendIfGuest(String user, String name)
        {
            //get friends
            String guests = db.getIfGuest(user, name);
            byte[] data = Encoding.ASCII.GetBytes(guests);
            byte[] len = new byte[2];
            len[0] = (byte)(data.Length >> 8);
            len[1] = (byte)(data.Length & 0xFF);
        ...
        public static void sendGuests(String user, String name)
        {
            //get friends
            String guests = db.getGuests(name);
            byte[] data = Encoding.ASCII.GetBytes(guests);
            byte[] len = new byte[2];
            len[0] = (byte)(data.Length >> 8);
            len[1] = (byte)(data.Length & 0xFF);
            ...
            try
            {
                ...
                if (bytesRead > 0)
                {
                    ...
                    switch (conn.buffer[0])
                    {
                        ...
                        case 14:
                            // addEvent
                            String eventName = p.getNext();
                            String uaser = p.getNext();
                            Console.WriteLine("nume " + eventName);
                            db.addEvent(eventName, uaser, p.getNext(), p.getNext(), Int32.Parse(p.getNext()));
                            break;

                        case 15:
                            // getGuests
                            sendGuests(p.getNext(), p.getNext());
                            break;

                        case 16:
                            // get If Guest
                            sendIfGuest(p.getNext(), p.getNext());
                            break;
                    }
