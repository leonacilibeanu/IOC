...
namespace Messenger
{
    ...
    {
        public String nume = "", tel = "", id;
        Boolean isAdd = false;
        public ViewOwnProfile(string id, string eventName)
        {
            InitializeComponent();
            ...
            bool friends = false;
            List<String> guests = Client.getGuests(eventName);

            foreach (String f in guests)
            {
                if (f.Equals(id))
                {
                    friends = true;
            ...
            if (this.id.Equals(Client.id))
                ...
                this.button1.Text = "Already guests";
            else
            {
