...
namespace Messenger
{
    ...
    public class Client
    {
        ...
        public static void addEvent(String name, String userId, String place, String date, Int32 isHost)
        {
            Client.currentEvent = name;
            Client.currentPlace = place;
            Client.currentDate = date;
            byte[] data = Encoding.ASCII.GetBytes('\0' + name + '\0' + userId + '\0' + place + '\0' + date + '\0' + isHost);
            data[0] = 14; // cred ca e ok
            serverSock.Send(data);
        }

        public static void getEventsForUser(String user)
        {
            byte[] len = new byte[2];
            serverSock.Receive(len, 2, 0);
            int length = (len[0] << 8) + len[1];
            int offset = 0, received;
            ...
            while (length > 0)
            {
                received = serverSock.Receive(data, offset, length, 0);
                offset += received;
                length -= received;
            }
        ...
        public static bool getIfHost(String eventName) {
            byte[] d = Encoding.ASCII.GetBytes('\0' + Client.currentUser + '\0' + eventName);
            d[0] = 16;
            serverSock.Send(d);
        ...
        public static List<String> getGuests(String eventName)
        {
            byte[] d = Encoding.ASCII.GetBytes('\0' + Client.currentUser + '\0' + eventName);
            d[0] = 15;
            serverSock.Send(d);
        ...
        public static void addGuestToEvent(String userId)
        {
            byte[] data = Encoding.ASCII.GetBytes('\0' + Client.currentEvent + '\0' + userId + '\0' + "" + '\0' + "" + '\0' + 0);
            data[0] = 14;
            serverSock.Send(data);
        }
